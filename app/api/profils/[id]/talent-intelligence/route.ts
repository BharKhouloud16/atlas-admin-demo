...
// ATLAS TALENT — Talent Intelligence V1 (Batch 5). Réutilise EXACTEMENT la
// même requête groupée et la même construction que GET
// /api/profils/[id]/intelligence (Candidate Intelligence, Batch 4) — ce
// module ne duplique aucune logique de lecture, il l'étend d'une seule
// transformation supplémentaire (construireTalentIntelligence). Réservé
// Admin, même isolation que le reste du module Talent (jamais exposé au
// Client/Ingénieur : révèle forces/faiblesses internes de staffing).
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  return NextResponse.json({ profilId: profil.id, nom: profil.nom, prenom: profil.prenom, talentIntelligence });
}
